...
struct RandGen
{
    ...
    __device__ double operator()(int idx)
    {
        thrust::default_random_engine randEng(idx);
        thrust::uniform_real_distribution<double> uniDist(A, B);
        randEng.discard(idx);
        double x = uniDist(randEng);
...
int main(int argc, char **argv)
{
    ...
    const double EXPECTED_VALUE = std::atof(argv[2]);

    // Transfer to device and compute the sum.
    auto t1 = std::chrono::high_resolution_clock::now();
    thrust::device_vector<double> d_vec(N * N);
    thrust::transform(
        thrust::make_counting_iterator(0),
        thrust::make_counting_iterator(N * N),
        d_vec.begin(),
        RandGen());
    double x = thrust::reduce(d_vec.begin(), d_vec.end(), 0.0, thrust::plus<double>());
    auto t2 = std::chrono::high_resolution_clock::now();
    auto ms_int = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1);
    std::cout << "Time performing device_vector mapping and reduction: " << ms_int.count() << "ms\n";

    double u0 = (B - A) * x / (N * N);
    double v = std::fabs(1 - (u0 / EXPECTED_VALUE));
